...
class Data:
    ...
    def build_gaz_pretrain_emb(self, emb_path, skip_first_row=False, separator=" "):
        ...
        self.pretrain_gaz_embedding, self.gaz_emb_dim = build_pretrain_embedding(emb_path, self.gaz_alphabet, skip_first_row, separator,
                                                                                 self.gaz_emb_dim,
                                                                                 self.norm_gaz_emb,isdic=True)

    def generate_instance(self, input_file, name, random_split=False):
        texts, ids, cut_num, mention_num = read_instance(input_file, self.gaz, self.char_alphabet, self.label_alphabet, self.gaz_alphabet, self.number_normalized, self.max_sentence_length)
        if name == "train":
            if random_split:
                random.seed(1)
                ...
                self.mention_num = mention_num
                print("train file include %d mention" % mention_num)
        elif name == "dev":
            self.dev_ids = ids
            self.dev_texts = texts
            self.dev_cut_num = cut_num
            self.mention_num = mention_num
            print("dev file include %d mention" % mention_num)
        elif name == "test":
            self.test_ids = ids
            self.test_texts = texts
            self.test_cut_num = cut_num
            self.mention_num = mention_num
            print("test file include %d mention" % mention_num)
        else:
